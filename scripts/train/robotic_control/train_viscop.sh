...
INIT_MODEL=/path/to/videollama3-image_7b_local # path to base VLM (for ViSCoP we use VideoLLaMA3 as the base VLM)

NUM_DATA_WORKERS=8
NUM_TRAIN_EPOCHS=3
LORA_TRAINING=False # we train the full LLM for robotic control tasks
...
RUN_NAME=viscop_qwen2.5_7b_Vima-DinBC-8k_train-ViSCoP_projector-CAv1-LLM

# Data Arguments
DATA_DIR=/path/to/datasets/robotics/
TRAINING_JSON="training_jsons/robotics/D-inBC-text-multi-train-8k-front.json"
MAX_FRAMES=180
...
torchrun --nnodes $WORLD_SIZE \
    ...
    --report_to $REPORT_TO \
    --run_name $RUN_NAME \
    --dataset_cache_dir /tmp/viscop_datasetcache \
