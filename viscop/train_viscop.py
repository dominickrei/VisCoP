...
def train(attn_implementation=None):
    ...
    config.interaction_module = 'cross_attention' # currently only support cross_attention
    ...
    if model_args.vision_encoder is not None:
        ...
        ###
        # Begin weight matching between SigLIP layers and interaction modules. Initialize the newly 
        # added interaction modules with SigLIP self-attention weights
        ###
        new_state_dict = {}
        for key, value in model.get_model().vision_encoder.vision_encoder.encoder.state_dict().items():
            if 'self_attn' in key:
                ...
                if corresponding_key in model.get_model().vision_encoder.vision_encoder.encoder.state_dict():
        ...
        # > initialize the newly added visual probe projector with pretrained weights of visual projector
        ...
        vlprocessor = ViSCoP_Processor(vision_encoder.image_processor, tokenizer, include_visual_tokens=model_args.include_visual_tokens, include_visual_probes=model_args.include_visual_probes, num_visual_probes=model_args.num_visual_probes)
